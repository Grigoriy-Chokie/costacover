...
(function(){
  ...
  theme.Variants = (function() {
    ...
    function Variants(options) {
      ...
      this.qtyProduct = this._getQtyProduct();
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _getQtyProduct: function() {
        return this.container.querySelector("input.js-qty__num") ? Number(this.container.querySelector("input.js-qty__num").value): 1
      },
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      init: function() {
        ...
        document.addEventListener('cart:updated', function(evt) {
          ...
          document.querySelector('.drawer__title-items-count').innerText = `(${cart.item_count})`;
        });
      ...
      initRemoveSelectors: function() {
        this.form.querySelectorAll(selectors.removeSelector).forEach(element => {
          const key = element.closest('.cart__item').dataset.key;
          if(key){
            element.addEventListener('click',(event)=>{
              event.preventDefault();          
              document.dispatchEvent(new CustomEvent('cart:quantity' + this.namespace, {
                detail: [key, "0", element.closest('.cart__item')]
              }));
            });
          }
        })
  ...
  theme.QtySelector = (function() {
    ...
    QtySelector.prototype = Object.assign({}, QtySelector.prototype, {
      init: function() {
        this.plus.addEventListener('click', function(evt) {
          var qty = this._getQty();
          this._change(qty + 1);
          const isProductTriger = evt.target.closest(".product__quantity");
          if (isProductTriger) {
            document.dispatchEvent(new CustomEvent('update:price', { bubbles: true, detail: this._getQty() }));
          }
        }.bind(this));
  
        this.minus.addEventListener('click', function(evt) {
          var qty = this._getQty();
          this._change(qty - 1);
          const isProductTriger = evt.target.closest(".product__quantity");
          if (isProductTriger) {
...
class SwiperConfigurator {
    ...
    parseConfig() {
        const { dataset: { config, desktop, mobile, observer } } = this.element;
    ...
    shouldDisableSwiper() {
        return (this.options.mobile && window.innerWidth > 768) || (this.options.desktop && window.innerWidth <= 768);
    }
  ...
  theme.Collection = (function() {
    ...
    initSort: function() {
        this.sortSelects = document.querySelectorAll(selectors.sortSelects);
        
        if (!this.sortSelects.length) {
          const fallbackSelect = document.querySelector(selectors.sortSelect);
          if (fallbackSelect) {
            this.sortSelects = [fallbackSelect];
          }
        }
        ...
        if (this.sortSelects.length) {
          ...
          this.sortSelects.forEach((select, index) => {
            select.addEventListener('change', (evt) => {
              this.onSortChange(evt, index);
            });
          });
        }
      },
  
      getSortValue: function(selectIndex = 0) {
        if (this.sortSelects.length > 0 && this.sortSelects[selectIndex]) {
          return this.sortSelects[selectIndex].value || this.defaultSort;
        }
        return this.defaultSort;
      },
  
      getDefaultSortValue: function() {
        for (let select of this.sortSelects) {
          const defaultValue = select.getAttribute('data-default-sortby');
          if (defaultValue) {
            return defaultValue;
          }
        }
      ...
      syncAllSelects: function(excludeIndex = -1) {
        if (this.sortSelects.length <= 1) return;
        
        const currentValue = this.getSortValue();
      ...
      onSortChange: function(evt, initiatorIndex) {
        this.syncAllSelects(initiatorIndex);
        
        this.queryParams = new URLSearchParams(window.location.search);
  
        this.queryParams.set('sort_by', this.getSortValue(initiatorIndex));
      ...
      setFilterStickyPosition: function() {
        var headerHeight = document.querySelector('.site-header').offsetHeight;
        var filtersElement = document.querySelector(selectors.filters);
        filtersElement.style.top = headerHeight + 'px';
        ...
        var filtersOffsetTop = filtersElement.offsetTop;
        
        function handleScroll() {
          if (window.pageYOffset > filtersOffsetTop) {
            filtersElement.classList.add('scrolled');
          } else {
            filtersElement.classList.remove('scrolled');
          }
        }
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      document.addEventListener("update:price", (event) => {
        const {currentVariant} = this.variants;
        const currentQty = event.detail;
        const functionUpdate = this.updatePrice.bind(this);
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      updatePrice: function(evt) {
        var variant = evt.detail.variant;
        var qtyProductDetails = evt.detail.qty ? Number(evt.detail.qty) : 1;
        var qtyProduct = qtyProductDetails == 0 ? 1 : qtyProductDetails;
  
        if (variant) {
          // Regular price
          this.cache.price.innerHTML = theme.Currency.formatMoney((variant.price * qtyProduct ), theme.settings.moneyFormat);
  
          // Sale price, if necessary
          if (variant.compare_at_price > variant.price) {
            this.cache.comparePrice.innerHTML = theme.Currency.formatMoney((variant.compare_at_price * qtyProduct), theme.settings.moneyFormat);
            this.cache.priceWrapper.classList.remove(classes.hidden);
            this.cache.price.classList.add(classes.onSale);
            this.cache.comparePriceA11y.setAttribute('aria-hidden', 'false');
            this.cache.priceA11y.setAttribute('aria-hidden', 'false');
  
            var savings = (variant.compare_at_price * qtyProduct) - (variant.price * qtyProduct);
            
            if (theme.settings.saveType == 'percent') {
              savings = Math.round(((savings) * 100) / (variant.compare_at_price * qtyProduct )) + '%';
            } else {
  ...
  constructor() {
    super();
    this.modalId = this.dataset.modalOpenerId;
    if (this.modalId) {
      this.modalDrawer = document.querySelector(`[data-modal-id="${this.modalId}"]`);
      if (this.modalDrawer) {
        this.modalCloseButtons = this.modalDrawer.querySelectorAll('[data-modal-close]');
        this.initListeners();
      }
    }
  }
  ...
  focusFirstField = () => {
    const el = this.modalDrawer.querySelector(
  ...
  deferFocus = () => {
    requestAnimationFrame(() => requestAnimationFrame(this.focusFirstField));
    this.modalDrawer.addEventListener('transitionend', this.focusFirstField, { once: true });
    this.modalDrawer.addEventListener('animationend', this.focusFirstField, { once: true });
  };

  initListeners() {
    this.addEventListener('click', (e) => {
      this.modalDrawer.setAttribute('open', 'true');
      document.documentElement.classList.add('lock-scroll');
      this.deferFocus();
      if (this.onOpen !== undefined) this.onOpen() || true;
    });

    this.modalCloseButtons.forEach(closeButton => {
      closeButton.addEventListener('click', () => {
        this.modalDrawer.removeAttribute('open');
        document.documentElement.classList.remove('lock-scroll');
        if (this.onClose !== undefined) this.onClose() || true;
      });
    });
  }
}

  class SmartSeach extends HTMLElement {
    constructor() {
      super();
      this.inputWrapper = document.querySelector(`[data-smart-search="${this.id}"]`);
      this.shopUrl = this.getAttribute('data-shop-url');
      if (this.inputWrapper) {
        this.searchButton = this.inputWrapper.querySelector('.search-button');
        this.searchInput = this.inputWrapper.querySelector('input[type="text"]') || this.drawer;
        if (this.searchInput) {
          this.onInputListener.call(this);
          this.onModalOpenListener.call(this);
        }
        if (this.searchButton && this.searchInput) {
          this.searchSubmitListener.call(this);
        }
      }
    }

    onModalOpenListener() {
      const modalDrawer = this.closest('[data-modal-id]');
      if(modalDrawer) {
        const modalOpener = document.querySelector(`[data-modal-opener-id="${modalDrawer.getAttribute('data-modal-id')}"]`);
        if(modalOpener) {
          modalOpener.onOpen = () => {
      ...
      this.searchInput.addEventListener('input', (event) => {
        ...
        timeoutId = setTimeout(() => {
          if(event.target.value == '') {
            ...
            this.fetchContent(event.target.value);
          }
        }, 500);
      })
    }
    ...
    fetchContent(terms) {
      fetch(`${this.shopUrl}/search/suggest?q=${terms}&resources[type]=query,product,collection,article&resources[limit]=8&resources[limit_scope]=each&section_id=smart-search`)
        .then(data => {
          return data.text();
        })
        .then(data => {
          const html = new DOMParser().parseFromString(data, 'text/html');
          const smartSearchContent = html.querySelector('.smart-search-wrapper');
          if (smartSearchContent) {
            this.innerHTML = smartSearchContent.innerHTML;
            const sliderElements = this.querySelectorAll("[data-slider-selector]");
          if (sliderElements.length > 0) {
            SwiperConfigurator.appendSwiperFiles()
              .then(() => {
                sliderElements.forEach(slider => new SwiperConfigurator(slider));
    ...
    searchSubmitListener() {
      this.searchButton.addEventListener('click', (event) => {
          event.preventDefault();
          window.location.assign(`${this.shopUrl}/${this.searchButton.getAttribute('href')}?q=${this.searchInput.value}&type=product`);
        });
  ...
  DOMready(function(){
    ...
    customElements.define('smart-search', SmartSeach);
  });
...
class GridSelector extends HTMLElement {
  ...
  handleRadioChange(event) {
    ...
    const gridWrapper = this.closest('.collection-grid__wrapper') || 
                        this.closest('.collection').querySelector('.collection-grid__wrapper');
    
    if (gridWrapper) {
      gridWrapper.classList.remove('grid-grid', 'grid-stacked');
      
      gridWrapper.classList.add(selectedRadio.value);
    }
  }
}
